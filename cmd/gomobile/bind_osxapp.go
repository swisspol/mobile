...
func goOSXBind(pkg *build.Package) error {
	...
	err = writeFile(mainFile, func(w io.Writer) error {
		return osxBindTmpl.Execute(w, "../go_"+name)
	})
	if err != nil {
		return fmt.Errorf("failed to create the binding package for OS X: %v", err)
	}
	...
	for _, env := range [][]string{macAmd64Env} {
		arch := archClang(getenv(env, "GOARCH"))
		path, err := goOSXBindArchive(name, mainFile, env)
		if err != nil {
	...
	return ioutil.WriteFile(buildO+"/Resources/Info.plist", []byte(osxBindInfoPlist), 0666)
}
...
func goOSXBindArchive(name, path string, env []string) (string, error) {
...
var osxBindTmpl = template.Must(template.New("osx.go").Parse(`
package main
